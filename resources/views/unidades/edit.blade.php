...
<div class="max-w-4xl mx-auto px-4 py-8">
    <div class="bg-white shadow-sm border border-gray-200 rounded-lg overflow-hidden">
        <div class="bg-gradient-to-r from-blue-600 to-blue-700 px-8 py-6">
            <h2 class="text-2xl font-semibold text-white">Editar Unidad Académica</h2>
            <p class="text-blue-100 text-sm mt-1">Modifique la información de la unidad</p>
        </div>

        <form method="POST" action="{{ route('unidades.update', $unidad->id_unidad) }}" class="p-8">
            @csrf @method('PUT')

            <div class="space-y-6">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Nombre</label>
                    <input type="text" name="nombre" value="{{ $unidad->nombre }}"
