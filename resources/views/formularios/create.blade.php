...
<div class="bg-white shadow-lg rounded-xl p-6 max-w-2xl mx-auto">
  <h2 class="text-2xl font-semibold text-indigo-700 mb-4">Registrar Formulario</h2>
  ...
  <form method="POST" action="{{ route('formularios.store') }}" class="space-y-4">
    ...
    <div>
      ...
      <input type="text" name="titulo" class="w-full border rounded-lg p-2 focus:ring-indigo-500" required>
    </div>
    <div>
    ...
    <div>
      ...
      <input type="date" name="fecha_creacion" value="{{ date('Y-m-d') }}" class="w-full border rounded-lg p-2 focus:ring-indigo-500" required>
    </div>
